...
def ARTIFACT_DIR= "jenkins_mds_build"
def NEXUS_REPO = 'http://34.214.232.47:8081/nexus/content/repositories/test/mds'
def PROJECT_NAME='Accenture'
def PROJECT_GROUP = 'accenture.test'
def APPLICATION_NAME='mds'
...
echo 'moving dirs for mds to jenkins directory'
cp -r ./Integrations/MDS ./${ARTIFACT_DIR}/Integrations
echo 'creating mds tarball...'
tar -zcf ./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}.tar.gz ./${ARTIFACT_DIR}/Integrations/MDS
echo 'created mds build tarball'
					"""
					echo '-------------------------------------------------------------------------------------------------------'
