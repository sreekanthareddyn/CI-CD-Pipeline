...
def NEXUS_REPO = 'http://34.214.232.47:8081/nexus/content/repositories/test/oic'
...
pipeline {
    ...
    stages {
        ...
        stage ('QualityCheck') {
            steps {
                timestamps {
					echo 'code quality analysis for oic using geritt'
				}
			}
        }
        stage ('CodeAnalysis'){
            ...
            steps {
                timestamps {
					...
					sh """#!/bin/bash +x
						...
						mkdir temp
						echo 'copung everything in /oic to temp'
						cp ./Integrations/oic/*/ temp

						echo 'moving  files for oic to temp'
						cp `ls ./Integrations/oic/*.IAR | tail -n 2` ./temp/
						
						echo 'files in temp to tarball:'
						ls -l ./temp

						echo 'creating oic tarball...'
						tar -zcf ${PROJECT_NAME}-${APPLICATION_NAME}-1.0.${BUILD_NUMBER}.tar.gz ./temp
						echo 'created oic build tarball'
                ...
                timestamps {
					...
					nexusArtifactUploader(
						...
						artifacts: [
							...
							type: 'tar.gz',
							file: "${PROJECT_NAME}-${APPLICATION_NAME}-1.0.${BUILD_NUMBER}.tar.gz"]
						], 
					echo 'Removing local build artefact...'
					sh "rm ${PROJECT_NAME}-${APPLICATION_NAME}-1.0.${BUILD_NUMBER}.tar.gz"
        ...
        stage('SCM Post Build Tag') {
            steps {
                timestamps {
					...
					echo '-------------------------------------------------------------------------------------------------------'
                    withCredentials([usernamePassword(credentialsId: params.GIT_USER_CRED, passwordVariable: 'SCM_PASSWORD', usernameVariable: 'SCM_USERNAME')]) {
                        sh "git config user.email ${GIT_USER_EMAIL}"
                        sh "git config user.name '${GIT_USER_NAME}'"
                        sh "git tag -a TAG-${BUILD_TAG} -m '${PROJECT_NAME}-${APPLICATION_NAME} Post Build TAG ${BUILD_URL}'"
                        sh "git push ssh://${SCM_USERNAME}:${SCM_PASSWORD}@${GIT_URL}/${GIT_REPO_PATH}/${GIT_REPO_NAME}.git --tags"
                    }
					echo '-------------------------------------------------------------------------------------------------------'
