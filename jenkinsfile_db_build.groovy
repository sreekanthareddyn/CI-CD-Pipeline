...
def ARTIFACT_DIR= "jenkins_db_build"
def NEXUS_REPO = 'http://34.214.232.47:8081/nexus/content/repositories/test/db'
def PROJECT_NAME='Accenture'
def PROJECT_GROUP = 'accenture.test'
def APPLICATION_NAME='db'
...
echo 'moving dirs for db to jenkins directory'
cp -r ./Integrations/DB ./${ARTIFACT_DIR}/Integrations
echo 'creating db tarball...'
tar -zcf ./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}.tar.gz ./${ARTIFACT_DIR}/Integrations/DB
echo 'created db build tarball'
					"""
					echo '-------------------------------------------------------------------------------------------------------'
