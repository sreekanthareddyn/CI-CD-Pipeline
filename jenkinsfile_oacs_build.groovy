...
def ARTIFACT_DIR= "jenkins_oacs_build"
def NEXUS_REPO = 'http://34.214.232.47:8081/nexus/content/repositories/test/oacs'
...
mkdir ./${ARTIFACT_DIR} && mkdir ./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER} && mkdir ./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}/Reporting
echo 'moving dirs for OACS to jenkins directory'
cp -r ./Reporting/OACS ./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}/Reporting

echo 'moving rpd files for OACS to jenkins directory'
rm ./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}/Reporting/OACS/*.rpd
cp `ls ./Reporting/OACS/*.rpd | tail -n 1` ./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}/Reporting/OACS/
cd ./${ARTIFACT_DIR}
echo 'creating oacs tarball...'
tar -zcf ${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}.tar.gz ${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}
echo 'created oacs build tarball'
					"""
					echo '-------------------------------------------------------------------------------------------------------'
