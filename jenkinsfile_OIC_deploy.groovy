    ...
    dir=`echo "\$d" | sed 's!.*/!!'`
        if [[ \$dir = Lookup ]]; then
            for file in *; do
                if [[ -f \$file ]]; then
                    if [[ \$file = *.csv ]]; then
                        status_code=\$(curl -k -v -X POST -u ${OIC_Username}:${OIC_Password} -F file=@./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}/Integrations/OIC/Integration/\$dir/\$file -F type=application/octet-stream ${OIC_url}/${ImpprtAddLookup})
        ...
        for file in *; do
            if [[ -f \$file ]]; then
                if [[ \$file = *.iar ]]; then
                    IntegrationID="\${file%.*}"
                    echo "\${IntegrationID}"
                    status_code=\$(curl -u ${OIC_Username}:${OIC_Password}  -H "Accept: application/json" ${OIC_url}/${ImportAddIar} -X POST -F "file=@./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}/Integrations/OIC/Integration/\$dir/\$file"; type=application/octet-stream)
                    if [[ \$status_code != 200 ]]; then
                        echo "Iar deployment failed"
                        exit 2;
                    fi
                    connection_status_code=\$(curl -X POST -u ${OIC_Username}:${OIC_Password} -H "X-HTTP-Method-Override:PATCH" -H "Content-Type:application/json" -d @./repository/${IAR_CONN_PROP} ${OIC_url}/${UpdateConnection}/${IntegrationID})    
