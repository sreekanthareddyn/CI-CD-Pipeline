...
def ARTIFACT_DIR= "jenkins_bip_build"
def NEXUS_REPO = 'http://34.214.232.47:8081/nexus/content/repositories/test/bip'
def PROJECT_NAME='Accenture'
def PROJECT_GROUP = 'accenture.test'
def APPLICATION_NAME='bip'
...
echo 'moving dirs for bip to jenkins directory'
cp -r ./Reporting/BIP ./${ARTIFACT_DIR}/Reporting
echo 'creating bip tarball...'
tar -zcf ./${ARTIFACT_DIR}/${APPLICATION_NAME}-${ARTIFACT_VERSION}.${BUILD_NUMBER}.tar.gz ./${ARTIFACT_DIR}/Reporting/BIP
echo 'created bip build tarball'
					"""
					echo '-------------------------------------------------------------------------------------------------------'
